  ...
  def rule2RDF(rule: Rule, label: RDFNode, rdf: RDFBuilder): RDFBuilder = {
    rule match {
      ...
      case OrRule(r1, r2) => {
        ...
        rdf.addTriple(RDFTriple(label, sh_constraint, choiceNode))
        rdf.addTriple(RDFTriple(choiceNode, rdf_type, sh_OrConstraint))
        val list = rdflist(List(o1,o2),rdf)
        rdf.addTriple(RDFTriple(choiceNode, sh_shapes, list))
        rule2RDF(r1, o1, rdf)
        rule2RDF(r2, o2, rdf)
      }
